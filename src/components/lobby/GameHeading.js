...
const GameHeading = ({ gameTitle, gameStatus, gameDescription, userData, gameData }) => {
  ...
  <div className="flex flex-col items-start justify-between gap-x-8 gap-y-4 bg-gray-700/10 px-4 py-4 sm:flex-row sm:items-center sm:px-6 lg:px-8">
    ...
    {gameStatus !== 'active' && <>
    {hidden && userIsGm && <div className="order-first flex-none rounded-full bg-indigo-400/10 px-2 py-1 text-xs font-medium text-indigo-400 ring-1 ring-inset ring-indigo-400/30 sm:order-none">
      Schedule a session
    </div>}
    {hidden && userIsGm && <Button href={`/prep/${gameData.id}`} className="order-first flex-none rounded-full bg-indigo-400/10 px-2 py-1 text-xs font-medium text-indigo-400 ring-1 ring-inset ring-indigo-400/30 sm:order-none">
    ...
    {hidden && <Button href={`/join/${gameData.id}`} className="order-first flex-none rounded-full bg-indigo-400/10 px-2 py-1 text-xs font-medium text-indigo-400 ring-1 ring-inset ring-indigo-400/30 sm:order-none">
      Join the Game
    </Button>}
    </>}
