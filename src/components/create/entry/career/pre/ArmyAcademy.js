...
import { commissionChance } from '../standard/Army';
...
export const entryChance = (character, entryModifier = 0) => {
    const { endurance } = calculateBonuses(character.coreCharacteristics);
    const term = character.lifePaths.length + 1;
    if (term > 3) return 0;
    const termModifier = term === 2 ? -2 : term === 3 ? -4 : 0;
    const netModifier = termModifier + endurance + entryModifier;
    const result = probabilizerBasic(netModifier, 7 );
